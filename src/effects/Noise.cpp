...
const byte p[] = {   151,160,137,91,90, 15,131, 13,201,95,96,
...
Noise::Noise(int speed, int smooth, int contrast, int level, Led::Color lowerColor, Led::Color upperColor): speedFactor(speed * 0.00001f),
                                                                                                            smoothingFactor(smooth * 0.001f),
                                                                                                            contrastFactor(contrast),
                                                                                                            shiftFactor(level * 0.02f - 1),
                                                                                                            lowerColor(lowerColor),
                                                                                                            upperColor(upperColor) {}

void Noise::update(){
  double tick = effectManager.getUpdateCount()*speedFactor;
  for(int i = 0; i < Lamp.numPixels(); i++){
    double value = fnoise(tick, i*smoothingFactor, 0);

    Led::Color lerpColor = lerp(value, lowerColor, upperColor);
    Lamp.setPixelColor(i, rawColor(lerpColor));
  }

  Lamp.show();
}

String Noise::getName() const{
    return "noise";
}
...
double Noise::fnoise(double x, double y, double z){
  return 1 / (1 + exp((-contrastFactor) * (pnoise(x, y, z) + shiftFactor)));
}

double Noise::pnoise(double x, double y, double z){ //-1 to 1
